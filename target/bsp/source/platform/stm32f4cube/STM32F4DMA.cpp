...
#include <platform/stm32f4cube/STM32F4DMA.hpp>
#include <cassert>
...
void STM32F4DMA::setupDataCopy(const BlitOp& blitOp)
{
    ...
    bool blendingImage   = ((blitOp.operation == BLIT_OP_COPY_WITH_ALPHA) || (blitOp.operation == BLIT_OP_COPY_ARGB8888) || (blitOp.operation == BLIT_OP_COPY_ARGB8888_WITH_ALPHA));
...
void STM32F4DMA::setupDataFill(const BlitOp& blitOp)
{
    ...
    else
    {
        ...
        Dma2dHandle.Init.Mode = dma2dTransferMode;
        Dma2dHandle.Init.ColorMode =  dma2dColorMode;
        Dma2dHandle.Init.OutputOffset = blitOp.dstLoopStride - blitOp.nSteps;
...
void STM32F4DMA::setupFillWithAlpha(const BlitOp& blitOp)
{
    uint16_t pos_x        = 0;
    uint16_t pos_y        = 0;
    ...
    hdma2d->Instance->BGPFCCR = (((bitDepth == 16) ? CM_RGB565 : CM_RGB888) | (transparency << ((uint32_t)POSITION_VAL(DMA2D_BGPFCCR_ALPHA))));
